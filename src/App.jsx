...
const API = process.env.VITE_API_URL || "https://insight-b91v.onrender.com/api/upload";
  ...
  return (
    <div className="flex items-center justify-between mb-6">
      ...
      <div className="flex items-center gap-4">
        <button className="px-3 py-1 rounded-md card muted" onClick={()=>window.open('https://github.com','_blank')}>Source</button>
...
function ThemeToggle({theme, setTheme}){
  ...
  return (
    <button onClick={toggle} className="flex items-center gap-2 card px-3 py-1">
      ...
      <div className={clsx('w-12 h-6 rounded-full p-1', theme==='dark' ? 'bg-accent' : 'bg-gray-300')}>
        <div className={clsx('w-4 h-4 rounded-full bg-white transform', theme==='dark' ? 'translate-x-6' : '')} />
      </div>
    </button>
  )
}
  ...
  return (
    <div className="card mb-6">
      <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{delay:0.05}}>
        ...
        <p className="muted mb-4">Drag & drop or click to choose a CSV. The backend analyzes and returns KPIs.</p>
        <div className="flex gap-4 items-center">
          <label className="flex-1 p-6 border-2 border-dashed rounded-md flex flex-col items-center justify-center cursor-pointer"
                 onDragOver={e=>e.preventDefault()} onDrop={e=>{ e.preventDefault(); const f = e.dataTransfer.files[0]; setFile(f); }}>
          ...
          <div className="w-48">
            <button className="w-full py-2 rounded-md bg-accent text-white" onClick={upload} disabled={loading}>
...
function KPIGrid({summary}){
  ...
  const {rows, columns, missing_values, outliers, kpis} = summary
  return (
    <motion.div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6" initial={{opacity:0}} animate={{opacity:1}}>
      ...
      <div className="card">
        ...
        <div className="text-2xl font-semibold">{columns}</div>
      </div>
      <div className="card">
        <div className="muted">Missing</div>
        <div className="text-2xl font-semibold">{missing_values}</div>
      </div>
      <div className="card">
        <div className="muted">Outliers</div>
        <div className="text-2xl font-semibold">{outliers?.outlier_rows ?? 0}</div>
      </div>
    </motion.div>
  )
}
...
function ChartPane({sample}){
  if(!sample || Object.keys(sample).length===0) return <div className="card muted">No numeric columns to chart.</div>
    ...
    <div className="card">
      ...
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={data}>
          ...
          <Tooltip />
          <Legend />
          {cols.map((c,i)=> <Line key={c} type="monotone" dataKey={c} stroke={colors[i%colors.length]} strokeWidth={2} dot={{r:2}} />)}
        </LineChart>
      </ResponsiveContainer>
    </div>
  )
  ...
  return (
    <div className="container">
      ...
      <div className="mt-6 card muted">
        ...
        <pre className="text-sm" style={{maxHeight:200,overflow:'auto'}}>{result ? JSON.stringify(result.sample || result.summary, null, 2) : 'No data yet'}</pre>
      </div>
